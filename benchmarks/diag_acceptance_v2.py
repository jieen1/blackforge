"""Diagnostic v2: acceptance with draft KV reset between tests."""
import os, sys, time
...
def make_prompt(tokenizer, target_len):
    base = "The quick brown fox jumps over the lazy dog. In a world of artificial intelligence and machine learning, the importance of efficient inference cannot be overstated. "
    tokens = []
...
def main():
    print("=" * 60)
    print("Acceptance v2: WITH draft KV reset between tests")
    print("=" * 60)
    print("\nLoading model...")
    ...
    for ctx_len in [512, 2048, 4096, 8192, 16384]:
        ...
        accepted, drafted = run_eager_steps(engine, backend, slot, first_token, aux_hs, actual_len, n_steps)
        rate = accepted / max(drafted, 1)
        tok_per_step = 1 + accepted / max(n_steps, 1)
        print(f"  {actual_len:>6} tokens: accept={rate:.1%} ({accepted}/{drafted}), tok/step={tok_per_step:.2f}, prefill={t_prefill:.1f}s")
        backend.reset_slot(slot)
    print("\nDone.")
